...
import { Folder, GraduationCap, ArrowRight, Inbox } from "lucide-react";
import type { FolderItem } from "../../hooks/useDashboardData";
...
export default function RecentFolders({ folders, loading = false }: RecentFoldersProps) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow duration-200 hover:shadow-md">
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
        <div>
          <h3 className="text-base font-semibold text-slate-900">Recent Folders</h3>
          <p className="mt-1 text-sm text-slate-500">Latest content folders</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center px-6 py-14 text-center">
          ...
          <p className="mt-4 text-sm font-medium text-slate-900">No folders yet</p>
              ...
              <div className="flex items-center gap-4 px-6 py-4 transition hover:bg-slate-50/80">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-xs text-slate-500">
                    {item.grade?.name ?? "No Grade"} - {item.class?.name ?? "No Class"}
                  </p>
                </div>
                ...
                <div className="hidden shrink-0 text-right sm:block">
                  <div className="flex items-center gap-1 text-xs text-slate-500">
                    ...
                    <span>{item.grade?.name ?? "No Grade"}</span>
                  </div>
